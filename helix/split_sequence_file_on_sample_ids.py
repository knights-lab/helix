...
def make_arg_parser():
    ...
    parser.add_argument('-o', '--output', help='Set the directory path of the output folder (default: cwd)', default=os.path.join(os.getcwd(), "sheared_bayes.txt"))
    parser.add_argument('-b', '--buffer', help="the buffer size", type=int)
...
def split_sequence_file_on_sample_ids(fasta, output_dir, buffer):
    for group in grouper(read_fasta(fasta), buffer):
...
def main():
    ...
    split_sequence_file_on_sample_ids(args.fasta, args.output, args.buffer)
